...
const Status = () => {
  ...
  const handleViewStatus = async (statusId) => {
    try {
      ...
      console.log("error to view status", error);
    }
  };
  ...
  const handleDeleteStatus = async (statusId) => {
    try {
      ...
      console.log("error to delete status", error);
 ...
 const handleStatusPreview = (contact, statusIndex = 0) => {
  ...
  if (selectedStatus?._id) {
    handleViewStatus(selectedStatus._id);
  } else {
    console.error("❌ No valid _id found for status:", selectedStatus);
  }
};
