...
const StatusPreview = ({
  ...
  // console.log("contact id is",contact.id,"current user id is",currentUser?._id,"is owner is",isOwner)
  useEffect(() => {
    setProgress(0);

    let current = 0;
    const interval = setInterval(() => {
      current += 2; //increase progress by 2%
      setProgress(current);
      if (current >= 100) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      className={`fixed inset-0 w-full h-full flex  bg-black items-center bg-opacity-90 justify-center z-50 `}
        ...
        >
          ...
          <div className="w-full h-full flex items-center justify-center">
            {currentStatus?.contentType === "text" ? (
              <div className="flex items-center justify-center w-full h-full text-center px-6">
                <p
                  className="text-2xl font-semibold text-white break-words leading-relaxed"
                  style={{
              ...
              <AnimatePresence>
                {showViewers && (
                  <motion.div
                    ...
                    className="mt-2 bg-black bg-opacity-70 rounded-lg p-4 max-h-48 overflow-y-auto"
                  >
                    {loading ? (
